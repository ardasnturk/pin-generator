...
export async function generatePins(
  ...
  const CLUSTER_TEXT_COLOR = resolveEnvValue(env, "CLUSTER_TEXT_COLOR") ?? "#000";
  const ICON_COLOR_ACTIVE = resolveEnvValue(env, "ICON_COLOR_ACTIVE") ?? "#FFF";
  const ICON_COLOR_DEFAULT = resolveEnvValue(env, "ICON_COLOR_DEFAULT") ?? "#000";
  ...
  if (iconSize <= 0 || iconRenderSize <= 0) {
    throw new Error(
      "ICON_SCALE and COMPOSITE_SCALE are too small; resulting icon size is 0."
    );
  }
  if (
    iconRenderSize >
    Math.round(Math.min(baseMetaDefault.width, baseMetaDefault.height) *
      COMPOSITE_SCALE)
  ) {
